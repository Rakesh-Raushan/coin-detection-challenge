...
```mermaid
%%{init: {'theme': 'base', 'themeVariables': { 'primaryColor': '#ECECFF', 'primaryBorderColor': '#9370DB', 'primaryTextColor': '#000'}}}%%
flowchart TB
    ...
    subgraph Service["Service Layer"]
        Detection["DetectionService<br/>(YOLO singleton)"]
        Geometry["GeometryService<br/>(Pure functions)"]
    end

    subgraph Persistence["Persistence Layer"]
        ORM["SQLModel (ORM)<br/>(Image, Coin)"]
